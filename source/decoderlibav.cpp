...
AudioFileDecoder::AudioFileDecoder(const QString &filePath,
                                   const int maxDuration)
    : filePathCh(nullptr),
      frameBufferSize(((AVCODEC_MAX_AUDIO_FRAME_SIZE * 3) / 2) *
                      sizeof(uint8_t)),
      audioStream(-1), badPacketCount(0), badPacketThreshold(max_bad_packets),
      codec(nullptr), fCtx(nullptr), cCtx(nullptr), dict(nullptr),
      rsCtx(nullptr) {
  // convert filepath
#ifdef Q_OS_WIN
  const wchar_t *filePathWc =
      reinterpret_cast<const wchar_t *>(filePath.constData());
  ...
  // open file
  int openInputResult =
      avformat_open_input(&fCtx, filePathCh, nullptr, nullptr);
  if (openInputResult != 0) {
    qWarning("Could not open file %s (%d)", filePathCh, openInputResult);
    free();
    throw KeyFinder::Exception(GuiStrings::getInstance()
                                   ->libavCouldNotOpenFile(openInputResult)
                                   .toUtf8()
                                   .constData());
  }
  ...
  if (avformat_find_stream_info(fCtx, nullptr) < 0) {
    ...
    free();
    throw KeyFinder::Exception(GuiStrings::getInstance()
                                   ->libavCouldNotFindStreamInformation()
                                   .toUtf8()
                                   .constData());
  }
  ...
  if (durationMinutes < twelve_hour_limit &&
      durationSeconds > maxDuration * seconds_in_minute) {
    qWarning("Duration of file %s (%d:%d) exceeds specified maximum (%d:00)",
             filePathCh, durationMinutes, durationSeconds % seconds_in_minute,
             maxDuration);
    free();
    throw KeyFinder::Exception(
        GuiStrings::getInstance()
            ->durationExceedsPreference(durationMinutes,
  ...
  if (codecOpenResult < 0) {
    qWarning("Could not open audio codec %s (%d) for file %s", codec->long_name,
             codecOpenResult, filePathCh);
    free();
    throw KeyFinder::Exception(
        GuiStrings::getInstance()
            ->libavCouldNotOpenCodec(codec->long_name, codecOpenResult)
            .toUtf8()
            .constData());
  }
  ...
  if (rsCtx == nullptr) {
    ...
    free();
    throw KeyFinder::Exception(GuiStrings::getInstance()
                                   ->libavCouldNotCreateResampleContext()
                                   .toUtf8()
                                   .constData());
  }

  qDebug("Decoder prepared for %s (%s, %d)", filePathCh,
         av_get_sample_fmt_name(cCtx->sample_fmt), cCtx->sample_rate);
}
...
void AudioFileDecoder::free() {
  ...
  if (cCtx != nullptr) {
    ...
    if (codecCloseResult < 0) {
      qCritical("Error closing audio codec: %s (%d)", codec->long_name,
                codecCloseResult);
    }
  }
  if (fCtx != nullptr)
  ...
  try {
    ...
    if (!decodePacket(&avpkt, audio)) {
      ...
      if (badPacketCount > badPacketThreshold) {
        av_free_packet(&avpkt);
        qWarning("Too many bad packets (%d) while decoding file %s",
                 badPacketCount, filePathCh);
        throw KeyFinder::Exception(
            GuiStrings::getInstance()
                ->libavTooManyBadPackets(badPacketThreshold)
                .toUtf8()
                .constData());
      }
    }
...
auto AudioFileDecoder::decodePacket(AVPacket *originalPacket,
                                    KeyFinder::AudioData *audio) -> bool {
  // copy packet so we can shift data pointer about without endangering garbage
  // collection
  AVPacket tempPacket;
  ...
  while (tempPacket.size > 0) {
    ...
    int bytesConsumed = 0;
    bytesConsumed =
        avcodec_decode_audio4(cCtx, dataBuffer, &dataSize, &tempPacket);
    ...
    if (cCtx->sample_fmt != AV_SAMPLE_FMT_S16) {
      ...
      resampleResult = audio_resample(rsCtx, (short *)frameBufferConverted,
                                      (short *)frameBuffer, newSamplesDecoded);
      if (resampleResult < 0) {
        throw KeyFinder::Exception(GuiStrings::getInstance()
                                       ->libavCouldNotResample()
                                       .toUtf8()
                                       .constData());
      }
