...
AVFileMetadata::AVFileMetadata(TagLib::FileRef *inFr, TagLib::File *f)
    : fr(inFr), genericFile(f) {}
NullFileMetadata::NullFileMetadata(TagLib::FileRef *fr, TagLib::File *g)
    : AVFileMetadata(fr, g) {}
FlacFileMetadata::FlacFileMetadata(TagLib::FileRef *fr, TagLib::File *g,
                                   TagLib::FLAC::File *s)
    : AVFileMetadata(fr, g) {
  flacFile = s;
}
MpegID3FileMetadata::MpegID3FileMetadata(TagLib::FileRef *fr, TagLib::File *g,
                                         TagLib::MPEG::File *s)
    : AVFileMetadata(fr, g) {
  mpegFile = s;
}
AiffID3FileMetadata::AiffID3FileMetadata(TagLib::FileRef *fr, TagLib::File *g,
                                         TagLib::RIFF::AIFF::File *s)
    : MpegID3FileMetadata(fr, g, NULL) {
  aiffFile = s;
}
WavID3FileMetadata::WavID3FileMetadata(TagLib::FileRef *fr, TagLib::File *g,
                                       TagLib::RIFF::WAV::File *s)
    : AiffID3FileMetadata(fr, g, NULL) {
  wavFile = s;
}
Mp4FileMetadata::Mp4FileMetadata(TagLib::FileRef *fr, TagLib::File *g,
                                 TagLib::MP4::File *s)
    : AVFileMetadata(fr, g) {
  mp4File = s;
}
AsfFileMetadata::AsfFileMetadata(TagLib::FileRef *fr, TagLib::File *g,
                                 TagLib::ASF::File *s)
    : AVFileMetadata(fr, g) {
  asfFile = s;
}
...
MetadataWriteResult
AVFileMetadata::writeKeyToMetadata(KeyFinder::key_t key,
  ...
  for (unsigned int i = 0; i < METADATA_TAG_T_COUNT; i++) {
    ...
    if ((metadata_tag_t)i == METADATA_TAG_KEY) {
      ...
      writeKeyByTagEnum(data.left(METADATA_CHARLIMIT_KEY), (metadata_tag_t)i,
                        result, prefs);
...
void AVFileMetadata::writeKeyByTagEnum(const QString &data, metadata_tag_t tag,
                                       MetadataWriteResult &result,
                                       const Preferences &prefs) {
  // Key field in ID3 holds only 3 chars; treat all Key fields as the same
  unsigned int charLimit =
      (tag == METADATA_TAG_KEY ? METADATA_CHARLIMIT_KEY
                               : METADATA_CHARLIMIT_OTHERS);
  QString newString = prefs.newString(data, getByTagEnum(tag), charLimit,
                                      prefs.getMetadataWriteByTagEnum(tag));
...
bool AVFileMetadata::setGrouping(const QString & /*grp*/) { return false; }

bool AVFileMetadata::setKey(const QString & /*key*/) { return false; }
...
bool NullFileMetadata::setTitle(const QString & /*tit*/) { return false; }

bool NullFileMetadata::setArtist(const QString & /*tit*/) { return false; }

bool NullFileMetadata::setAlbum(const QString & /*tit*/) { return false; }

bool NullFileMetadata::setComment(const QString & /*cmt*/) { return false; }
...
QString FlacFileMetadata::getComment() const {
  ...
  if (flacFile->xiphComment()->contains(keyXiphTagComment)) {
    TagLib::String out =
        flacFile->xiphComment()->fieldListMap()[keyXiphTagComment].toString();
    return QString::fromUtf8((out.toCString(true)));
...
bool FlacFileMetadata::setComment(const QString &cmt) {
  ...
  flacFile->xiphComment()->addField(
      keyXiphTagComment,
      TagLib::String(cmt.toUtf8().constData(), TagLib::String::UTF8), true);
  genericFile->save();
  return true;
}
...
bool FlacFileMetadata::setKey(const QString &key) {
  flacFile->xiphComment()->addField(
      keyXiphTagKey,
      TagLib::String(key.toUtf8().constData(), TagLib::String::UTF8), true);
  flacFile->save();
  return true;
}
...
bool MpegID3FileMetadata::hasId3v2_3Tag() const {
  ...
  if (!hasId3v2Tag())
    return false;
  if (mpegFile->ID3v2Tag()->header()->majorVersion() != 3)
    return false;
  return true;
}
...
bool MpegID3FileMetadata::hasId3v2_4Tag() const {
  ...
  if (!hasId3v2Tag())
    return false;
  if (mpegFile->ID3v2Tag()->header()->majorVersion() != 4)
    return false;
  return true;
}
...
QString
MpegID3FileMetadata::getGroupingId3(const TagLib::ID3v2::Tag *tag) const {
...
bool MpegID3FileMetadata::setTitle(const QString &tit) {
  ...
  if (hasId3v2Tag()) {
    mpegFile->ID3v2Tag()->setTitle(
        TagLib::String(tit.toUtf8().constData(), TagLib::String::UTF8));
    mpegFile->save(TagLib::MPEG::File::ID3v2, false,
...
void MpegID3FileMetadata::setITunesCommentId3(TagLib::ID3v2::Tag *tag,
                                              const QString &cmt) {
  if (tag->frameListMap().contains(keyId3TagiTunesComment)) {
    const TagLib::ID3v2::FrameList &comments =
        tag->frameListMap()[keyId3TagiTunesComment];
    bool found = false;
    for (TagLib::ID3v2::FrameList::ConstIterator it = comments.begin();
         it != comments.end(); it++) {
...
bool MpegID3FileMetadata::setGrouping(const QString &grp) {
  if (!hasId3v2Tag())
    return false; // ID3v1 doesn't support Grouping
  setGroupingId3(mpegFile->ID3v2Tag(), grp);
  mpegFile->save(TagLib::MPEG::File::ID3v2, true,
                 mpegFile->ID3v2Tag()->header()->majorVersion());
  return true;
}

bool MpegID3FileMetadata::setGroupingId3(TagLib::ID3v2::Tag *tag,
                                         const QString &grp) {
  TagLib::ID3v2::Frame *frm =
      new TagLib::ID3v2::TextIdentificationFrame(keyId3TagGrouping);
...
bool MpegID3FileMetadata::setKey(const QString &key) {
  if (!hasId3v2Tag())
    return false; // ID3v1 doesn't support Key
...
bool MpegID3FileMetadata::setKeyId3(TagLib::ID3v2::Tag *tag,
                                    const QString &key) {
  TagLib::ID3v2::Frame *frm =
      new TagLib::ID3v2::TextIdentificationFrame(keyId3TagKey);
...
QString WavID3FileMetadata::getKey() const { return getKeyId3(wavFile->tag()); }
...
QString AsfFileMetadata::getGrouping() const {
  if (!asfFile->tag()->attributeListMap().contains(keyAsfTagGrouping))
    return emptyString;
  TagLib::ASF::AttributeList l =
      asfFile->tag()->attributeListMap()[keyAsfTagGrouping];
  TagLib::String out = l.front().toString();
  return QString::fromUtf8((out.toCString(true)));
}
...
QString AsfFileMetadata::getKey() const {
  if (!asfFile->tag()->attributeListMap().contains(keyAsfTagKey))
    return emptyString;
  TagLib::ASF::AttributeList l =
      asfFile->tag()->attributeListMap()[keyAsfTagKey];
  TagLib::String out = l.front().toString();
  return QString::fromUtf8((out.toCString(true)));
}
