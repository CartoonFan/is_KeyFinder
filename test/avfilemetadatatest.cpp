...
void testPathReturnsValues(const QString &path, const QString &expectedTitle,
...
void testWritingToFile(const QString &path, metadata_tag_t tag) {
  ...
  for (unsigned int i = 0; i < METADATA_TAG_T_COUNT; i++) {
    QString chk =
        initFileMetadata->getByTagEnum(static_cast<metadata_tag_t>(i));
  ...
  for (unsigned int i = 0; i < METADATA_TAG_T_COUNT; i++) {
    ...
    QString chk =
        resetFileMetadata->getByTagEnum(static_cast<metadata_tag_t>(i));
    ASSERT_TRUE(chk == initValue || chk == noValue);
  }
  delete resetFileMetadata;
}

void testId3WritingForMp3s(const QString &path, bool v1, bool v2, bool v2_3,
                           bool v2_4) {
  AVFileMetadataFactory factory;
...
TEST(AVFileMetadataTest, ReadMissing) {
  testPathReturnsValues("noFileHere",
...
TEST(AVFileMetadataTest, ReadNull) {
  testPathReturnsValues("../is_KeyFinder/test-resources/notAV.pdf",
                        GuiStrings::getInstance()->notApplicable(),
                        GuiStrings::getInstance()->notApplicable(),
                        GuiStrings::getInstance()->notApplicable(),
                        GuiStrings::getInstance()->notApplicable(),
...
TEST(AVFileMetadataTest, ReadFlac) {
  testPathReturnsValues("../is_KeyFinder/test-resources/readTags/flac.flac",
                        "Title", "Artist", "Album", "Comment",
                        GuiStrings::getInstance()->notApplicable(), "Key");
}
...
TEST(AVFileMetadataTest, ReadMp3WithID3v1) {
  testPathReturnsValues(
      "../is_KeyFinder/test-resources/readTags/mp3 with id3 v1.mp3", "Title v1",
      "Artist v1", "Album v1", "Comment v1",
      GuiStrings::getInstance()->notApplicable(),
      GuiStrings::getInstance()->notApplicable());
}
...
TEST(AVFileMetadataTest, ReadMp3WithID3v2_3) {
  testPathReturnsValues(
      ...
      "Title v2.3", "Artist v2.3", "Album v2.3", "Comment v2.3",
      "Grouping v2.3", "2.3");
}
...
TEST(AVFileMetadataTest, ReadMp3WithID3v2_4) {
  testPathReturnsValues(
      ...
      "Title v2.4", "Artist v2.4", "Album v2.4", "Comment v2.4",
      "Grouping v2.4", "2.4");
}
...
TEST(AVFileMetadataTest, ReadAiff) {
  testPathReturnsValues("../is_KeyFinder/test-resources/readTags/aiff.aiff",
                        "Title", "Artist", "Album", "Comment", "Grouping",
                        "Key");
}

TEST(AVFileMetadataTest, ReadWav) {
  testPathReturnsValues("../is_KeyFinder/test-resources/readTags/wav.wav",
                        "Title", "Artist", "Album", "Comment", "Grouping",
                        "Key");
}

TEST(AVFileMetadataTest, ReadAlac) {
  testPathReturnsValues("../is_KeyFinder/test-resources/readTags/alac.m4a",
                        "Title", "Artist", "Album", "Comment", "Grouping",
                        "Key");
}

TEST(AVFileMetadataTest, ReadAac) {
  testPathReturnsValues("../is_KeyFinder/test-resources/readTags/aac.m4a",
                        "Title", "Artist", "Album", "Comment", "Grouping",
                        "Key");
}

TEST(AVFileMetadataTest, ReadWma) {
  testPathReturnsValues("../is_KeyFinder/test-resources/readTags/wma.wma",
                        "Title", "Artist", "Album", "Comment", "Grouping",
                        "Key");
}
...
TEST(AVFileMetadataTest, WriteMissing) {
  ...
  for (unsigned int i = 0; i < METADATA_TAG_T_COUNT; i++) {
    ...
    fileMetadata->writeKeyByTagEnum("ABC", static_cast<metadata_tag_t>(i),
                                    result, prefs);
    for (unsigned int j = 0; j < METADATA_TAG_T_COUNT; j++) {
...
TEST(AVFileMetadataTest, WriteNull) {
  Preferences prefs;
  AVFileMetadataFactory factory;
  AVFileMetadata *fileMetadata =
      factory.createAVFileMetadata("../is_KeyFinder/test-resources/notAV.pdf");
...
TEST(AVFileMetadataTest, WriteMp3WithID3v1) {
  testId3WritingForMp3s(
      "../is_KeyFinder/test-resources/writeTags/mp3 with id3 v1.mp3", true,
      false, false, false);
}

TEST(AVFileMetadataTest, WriteMp3WithID3v2_3) {
  testId3WritingForMp3s(
      "../is_KeyFinder/test-resources/writeTags/mp3 with id3 v2.3.mp3", false,
      true, true, false);
}

TEST(AVFileMetadataTest, WriteMp3WithID3v2_4) {
  testId3WritingForMp3s(
      "../is_KeyFinder/test-resources/writeTags/mp3 with id3 v2.4.mp3", false,
      true, false, true);
}
